...
The result of the backwards BIC model selection is the model: Y ~ X1 + X5 + X8 + X2 + X4 + X3.  Through the backwards selection process, we have removed the predictors "Course Aggregate" (X6) and "Fine Aggregate" (X7) and have found that a smaller model is a better fit with lower BIC.  
...
We will continue to run diagnostics by using residual plots to check our assumptions of the model.  
First, we will look at our Residual vs. Fitted plot to assess our assumption of linearity.
...
When using BIC as the criteion function, both the forward selection and backwards elination algorithm yielded the same model.  
